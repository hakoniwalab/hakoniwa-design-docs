...
- (Version 5.4) 実装拡張として「遅延観測を含む一般モデル」を追記(2025/3/18)

#   概要

箱庭は、複数のシミュレータを繋いで連携動作させるシミュレーションハブである。本簡易論文では、箱庭の中心モジュールである箱庭コア（以降コア）と各シミュレータ（以降アセット）間の時刻同期の考え方を公理的に定義し、それによって、各シミュレータが独自に動作していても、全アセットがあらかじめ設定された最大許容遅延時間内で同期されていること、あるパラメータ条件下でデッドロックが起きないこと、効率的なパラメータ設定を数学的に議論する。

- 安全性（許容遅延時間内にシミュレーション時刻が同期している）
- 進行性（デッドロック状態に陥らないパラメータの条件）
- 効率性（シミュレーションが効率的に進むパラメータの条件）

さらに、行われた定式化をシミュレーション実験結果と比較し、結論の正当性を確認する。
...
ウォール時刻 $t$ において、前シミュレーション時刻 $T_i$ と現在のコアシミュレーション時刻 $T_c$ から、新シミュレーション時刻 $T_i$ を決定する。

新シミュレーション時刻 $T_i$ は、前シミュレーション時刻を規定幅 $\Delta T_i$ だけ進めたと想定した値（想定新時刻 $T_i'$ ）が、コアの現時刻 $T_c$ を追い越さないように確定する。
...
T_i \leftarrow
\begin{cases}
T_i'  & (T_i'\leq T_c(t) の時) \ldots 進行 (a.1) \\
T_i (\text{as-is}) & (T_c \lt T_i' の時) \ldots 停留 (a.2) (※note: ここで T_c(t) まで進行させる案もある)
\end{cases}
$$

- アセット時刻更新における保存則

上記ルールによって、すべてのアセット $i$について以下の不等式が、任意のウォール時刻 $t$ について成り立つ。

$$
T_i  \leq T_c \quad (\forall i) \ldots (a.3)
$$

すなわち、

$$
\max_{i = 1 \ldots n} T_i \leq T_c \ldots (a.4)
$$
...
（note: ここでは、 $\Delta t_i$は固定長としているが、それが$t$によって変化する場合でも、ウォール時間点列として単調増加な数列、 $t = 0, t^{(1)}, t^{(2)}, \ldots$ を取れば同様の証明ができる。）


### コアのカウントアップ規則

コアも同様に、ウォール時刻 $t$ において、前シミュレーション時刻 $T_c$ と $n$ 個のアセットシミュレーション時刻 $T_i (i = 1 \ldots n)$ から、自身の新シミュレーション時刻 $T_c$ を決定する。

新シミュレーション時刻 $T_c$ は、前シミュレーション時刻を規定幅 $\Delta T_c$ だけ進めたと想定した値（想定新時刻 $T_c'$ ）が、どのアセットの現時刻 $T_i$ も、許容幅 $D_{max}$ を超えて追い越さないように確定する。（ここで、 $D_{max}$ は $\Delta T_c, \Delta T_i$ に比べて大きい必要がある：後述のデッドロック参照）。
...
$$
D'_{max} = \max_{i = 1 \ldots n}(T_c' - T_i)
$$
...
T_c \leftarrow
\begin{cases}
T_c'  & (D'_{max}  \leq D_{max} の時) \ldots 進行 (b.1) \\
T_c (\text{as-is}) & (D_{max} \lt D'_{max} の時) \ldots 停留 (b.2) 
\end{cases}
$$
...
$$
T_c \leq T_i + D_{max} \quad (\forall i) \ldots (b.3)
$$

すなわち、

$$
T_c \leq \min_{i = 1 \ldots n}  T_i + D_{max} \ldots (b.4)
$$
...
ここでは、 $\Delta t_i$ は固定長としているが、それが $t$ によって変化する場合でも、ウォール時間点列として単調増加な数列、 $t = 0, t^{(1)}, t^{(2)}, \ldots$ を取れば同様の証明ができる。
...
前述した2つの等式を再掲する。 これらは、ウォール時刻 $t \geq 0$ について恒等的に成立する。

- どのアセットのシミュレーション時刻も、コアのシミュレーション時刻より進んでいない。

$$
\max_{i = 1 \ldots n} T_i \leq T_c \ldots (c.1)
$$

- コアのシミュレーション時刻は、どのアセットのシミュレーション時刻より、最大許容遅延時間を超えて進んでいない。

$$
T_c \leq \min_{i = 1 \ldots n}  T_i + D_{max} \ldots (c.2)
$$
...
$$
\max_{i = 1 \ldots n} T_i \leq T_c \leq \min_{i = 1 \ldots n}  T_i + D_{max} \ldots (c.3)
$$


さらに、この不等式の中央にある $T_c$ を省くことで、

- 最も進んだアセットと最も遅れたアセットのシミュレーション時刻差は、高々最大許容遅延時間である、

$$
\max_{i = 1 \ldots n} T_i \leq \min_{i = 1 \ldots n}  T_i + D_{max} \ldots (c.4)
$$
...
$$
| T_i - T_j | \leq D_{max} \quad (\forall i, j) \ldots (c.5)
$$
...
そして、このゾーンは $D_{max} < \Delta T_i$ の場合に存在する。
...
もう一つ、デッドロックが起こらない十分条件が存在する。それは、$T_c, T_i$ がどちらかがどちらかの整数倍になっている場合である。
...
1.  **一般進行性条件の可視化**: $\Delta T_c + \Delta T_i > 100$ となるパラメータ領域（グラフの右上領域）が、広範囲にわたってデッドロック（赤色）となっている。これは、「一般進行性条件」が破られるとシステムが不安定になるという理論と完全に一致する。

2.  **割り切れる条件（救済策）の可視化**: 赤色のデッドロック領域の中に、点々と存在する明るい安定の島が確認できる。これらは、 $\Delta T_c$ が $\Delta T_i$ で割り切れる（ $\Delta T_c \bmod \Delta T_i = 0$ ）という割り切れる条件が満たされた点（例: $(\Delta T_c = 80, \Delta T_i = 40)$ , $(\Delta T_c = 100, \Delta T_i = 20)$ ）に対応しており、一般進行性条件が満たされなくても、この救済策によって進行性が保証される様子が明瞭に示されている。
...
$$
\Delta T_c + \Delta T_i + E_i^{tot} \le D_{max} \quad (\forall i)
$$

を用いる。等価に、最悪ケース表現として

$$
\Delta T_c + \max_i\left(\Delta T_i + E_i^{tot}\right) \le D_{max}
$$
...
$$
\Delta T_c + \Delta T_i + E_i^{tot} \le D_{max}
$$
...
- 安全性条件は、観測値ベース判定が保守的に働く限り維持される。
- 一方、進行性は、保証されづらくなる。 $\Delta T_c, \Delta T_i$ を遅延を見込んで小さめに設定する必要がある。目安は遅延がない場合の（遅延実時間/処理実時間 $+ 1$ ）分の一。
- 実用上は、理想モデルの条件を設計指針として用いつつ、現実モデルで実験検証を行う。
