...
- **$ 0 < \Delta T_i \leq D_{max} $** （全てのアセット $i$ について）
...
なお、この条件（ $\Delta T_c + \max_{i}(\Delta T_i) \leq D_{max}$ ）を満たせば、前述の「基本要件」は自動的に満たされる。
...
（※証明と実験の詳細は [liveness-experiment.md](liveness-experiment.md) を参照）
