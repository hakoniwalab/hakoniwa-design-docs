...
「$D_{max}$ は $\Delta T_c, \Delta T_i$ より大きい必要がある」 という注記は、進行性の観点で置いている。これを正確に議論する。カウントアップ規則の下で、アセットとコアの進行条件は以下である。

- アセット $i$ が進行するための必要十分条件（前述の(a.1)より）：

$$
\Delta T_i + T_i \leq T_c(t) \ldots (d.1)アセットがコアを追い越さない
$$

- コアが進行するための必要十分条件（前述の(b.1)より）：

$$
(T_c+\Delta T_c)-T_i(t) \leq D_{max} \quad (\forall i) \ldots (d.2) D_{max} を超えてコアが進まない
$$

すなわち、

$$
(T_c+\Delta T_c) - \min T_i(t) \leq D_{max} \ldots (d.2')
$$

## 全停止しないための一般条件（一般進行性条件）

コアと全アセットが同時に永久停留する状態（全停止デッドロック）を許さない、という要請は、任意の時点 $t$ で次を満たすことと同値である。
...
$$
T_c < T_i + \Delta T_i \quad (\forall i)
$$
...
各アセット $T_i$ の存在領域、開区間 $(T_c - \Delta T_i, T_c)$ を $T_c$の`後方デッドゾーン`(rear dead-zone)という（右端の $T_c$ を含んでいないことに注意）。この領域ではアセットは進むことができない。

この場合、要請よりコアが進行可能でなければならないので、すなわち (d.2)

$$
T_c + \Delta T_c-T_i \leq D_{max} \quad (\forall i)
$$

が必要である。

ここで、上の局面（後方デッドゾーンにあるアセットがある）では各 $T_c-T_i$ は $\Delta T_i$ より小さい任意の値を取り得ると仮定すると（この仮定が成り立たない場合は後述）、常に (d.2) を保証するには、$T_c-T_i$ の上界である $\Delta T_i$ においても不等式が成り立つ必要がある。よって
...
$$
\max \Delta T_i + \Delta T_c \leq D_{max} \ldots (e.1')
$$

が必要となる。これを、`一般進行性条件`と呼ぶ。これが成り立つように $\Delta T_i, \Delta T_c, D_{max}$ を設計すればよい。この条件が進行性を保証する十分条件であることは以下のように示せる。

### 進行性（デッドロック回避）の証明（(e.1) の十分性）

以下を仮定する。

1. 設計条件 (e.1) を満たすようにすべての $\Delta T_i$ を設定する。
...
$T_c - T_i < \Delta T_i$ を合わせると、

$$
T_c - T_i + \Delta T_c < D_{max} \quad (\forall i)
$$

が成り立つので、(d.2)

$$
T_c - T_i + \Delta T_c \leq D_{max} \quad (\forall i)
$$


が成り立ち、 **コアは進行できる**。したがって、この場合は全ノード同時停留ではない。
...
シミュレーション時刻 $T_i$ が、コア時刻 $T_c$ に対して開区間 $T_c - \Delta T_i < T_i < T_c$ の範囲内に存在する場合、そのアセットは自身のカウントアップ規則により必ず停留する。この領域を「後方デッドゾーン」と定義する。この領域幅を最大（ $\max \Delta T_i$ ）に見積もって安全側で評価したのが、一般進行性条件： $\max \Delta T_i + \Delta T_c \leq D_{max}$ (e.1) である。

### 2. 進行性を保証するもう一つの「割り切れる条件」

もしコアの時間間隔 $\Delta T_c$が、あるアセット`i`の時間間隔$\Delta T_i$の整数倍である（$\Delta T_c \mod{\Delta T_i} = 0$）ならば、そのアセット`i`は「後方デッドゾーン」に入ることが数学的に不可能となる。
これにより、たとえ一般進行性条件が満たされない（$D_{max}$が不足する）場合でも、この「割り切れる条件」が全アセットに対して満たされていればシステムはデッドロックしない。

割り切れる条件：
全アセットについて、
$$
\Delta T_c \mod{\Delta T_i} = 0
$$
が成り立てば、
