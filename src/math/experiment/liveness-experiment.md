...
一般進行性条件と割り切れる条件の両方を満たさないパラメータ設定では、その結果は実行タイミングに依存する。今回の実験では、同じカテゴリに属する設定でも、デッドロックに陥るケース（Setting 2.1, 2.2）と、偶然にも完走するケース（Setting 2.3）の両方が観測され、予測通りの不安定な挙動が確認できた。
